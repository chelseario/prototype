...
$.ui.ddmanager.prepareOffsets = function (t, event) {
    ...
    droppablesLoop: for (i = 0; i < m.length; i++) {
        ...
        proportions = { width: m[i].element[0].offsetWidth, height: m[i].element[0].offsetHeight - $(m[i].element[0]).offsetParent().contents().scrollTop() };
        console.log("offsetWidth: " + m[i].element[0].offsetWidth);
        console.log("scrollTop: " + m[i].element[0].scrollTop);
        console.log("height: " + (m[i].element[0].offsetHeight - $(m[i].element[0]).offsetParent().contents().scrollTop()).toString());
